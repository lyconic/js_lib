					...
					.live("mousedown", function (event) {
						...
						if(_this.settings.rules.multiple != false && _this.selected_arr.length > 1 && obj.children("a:eq(0)").hasClass("clicked")) {
							...
							if(counter > 0) {
								...
								var tmp2 = $('#allAccounts a.clicked').parent('.account').slice(0,10); //line added for salesguide
								if(_this.settings.languages.length > 0) tmp.find("a").not("." + _this.current_lang).hide();
								 //original jstree code line below
								//tree_component.drag_drop.drag_help.children("ul:eq(0)").append(tmp);
								tmp2.clone().appendTo(tree_component.drag_drop.drag_help.children("ul:eq(0)")); //line added for salesguide
								 //original jstree code line below
								...
								tree_component.drag_drop.drag_help.append('<div class="dragged-gradient"></div>');
							}
						}
						else {
