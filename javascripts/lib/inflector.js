...
/*
    ...
    singularize: function(word) {
      ...
      for (var i = 0; i < Inflector.Inflections.irregular.length; i++) {
          var singular = Inflector.Inflections.irregular[i][0];
          var plural   = Inflector.Inflections.irregular[i][1];
          if ((word.toLowerCase() == plural) || (word == singular)) {
              return singular;
          }
      }
 ...
 *
 *  Returns count with singular or plural noun depending on the value
 *  of count. Plural noun can optionally be given if it is irregular.
 *
 *  Example:
 *  pluralize(1, 'account')
 *  => '1 account'
...
function pluralize() {
  ...
  return count + " " + (1 == parseInt(count) ? singular : plural || Inflector.pluralize(singular));
}

/**
 *  Returns plural value of this according to Inflector.pluralize
 *
 *  Example:
 *  'account'.pluralize()
 *  => 'accounts'
 **/
String.prototype.pluralize = Inflector.pluralize.methodize();
String.prototype.singularize = Inflector.singularize.methodize();

/**
 *  Returns pluralized sentenced based on the value of this.
 *
 *  Example:
 *  var num = 5;
 *  num.pluralize('account');
 *  => '5 accounts'
 *
 *  Can't be used with inline number values, but that defeats the purpose anyway.
 *
 *  Example:
 *  3.pluralize('tooth', 'teeth')
 *  => Exception
 *
 *  Number(3).pluralize('tooth', 'teeth')
 *  => '3 teeth'
 **/
